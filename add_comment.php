...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql_post_check = "SELECT id, user_id FROM posts WHERE id = '$post_id'";
    ...
    if (empty($errors['comment']) && $post_id) {
        $add_comment = "INSERT INTO comments (comment, user_id, post_id) VALUES (?, ?, ?)";
        $stmt = db_get_prepare_stmt($connect, $add_comment, [$comment, $user_id, $post_id]);
