...
$sql_post_check = "SELECT id FROM posts WHERE id = '$post_id'";
...
if ($post_check && !$likes_check) {
    ...
    $stmt = db_get_prepare_stmt($connect, $add_like, [$user_id, $post_id]);
    $result = mysqli_stmt_execute($stmt);
} elseif ($post_check && $likes_check) {
    $delete_like = "DELETE FROM likes WHERE user_id = ? AND post_id = ?";
    $stmt = db_get_prepare_stmt($connect, $delete_like, [$user_id, $post_id]);
    $result = mysqli_stmt_execute($stmt);
}
if (!$referer) {
